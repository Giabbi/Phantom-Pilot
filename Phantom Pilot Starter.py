...
def car1Handler(dimensions, luck, nitro, power):
    ...
    while True:
        ...
        for line in iter(stdout.readline, ""): # Read bytes as they are sent from Car 1 
            '''global shell1
            if shell1 and "Car 2 got a power star! It has become invulnerable to crashes!" not in line and not eventStopper:
                stdin1, stdout1, stderr2 = car1.exec_command("cd \"bcm2835-1.70/Motor_Driver_HAT_Code/Motor_Driver_HAT_Code/Raspberry Pi/python\"\n python3 test.py", get_pty = True)
                eventStopper = True
                jump = True
                break'''
            ...
            if "Car 1 crashed!!!" in line: # Check if the car crashed
...
def car2Handler(dimensions, luck, nitro, power):
    ...
    while True:
        ...
        for line in iter(stdout.readline, ""): # Read bytes as they are sent from Car 2 
            ''' global shell2
            if shell2 and "Car 2 got a power star! It has become invulnerable to crashes!" not in line and not eventStopper:
            ...
            if "Car 2 crashed!!!" in line: # Check if the car crashed
...
def main():
    ...
    while not car1Finish and not car2Finish:
        process_queue()
